...
root@devnet-test-2:~#
```
* [Enable SSH access using password to the machine](https://askubuntu.com/questions/101670/how-can-i-allow-ssh-password-authentication-from-only-certain-ip-addresses) `vi /etc/ssh/sshd_config` and reboot the machine `sudo reboot`
...
* Update the project and build it using the relevant node branch: 
`git pull && gcloud auth configure-docker` 
`NODE=hackathon make build_node`
`make build_agent`
